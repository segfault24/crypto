...
uint32_t* sha_256_224(char* filename, int mode) {

	//uint32_t M[16]; // local copy of working message block
...
uint64_t* sha_512_384(char* filename, int mode) {
	filename++;mode++;
	return 0;
}
...
int main(int argc, char *argv[]) {
	...
	for(i=2; i<argc; i++) {
		...
		switch(mode) {
		case MODE_224:
			h = sha_256_224(argv[i], MODE_224);
			break;
		case MODE_256:
			h = sha_256_224(argv[i], MODE_256);
			break;
		case MODE_384:
			h = sha_512_384(argv[i], MODE_384);
			break;
		case MODE_512:
			h = sha_512_384(argv[i], MODE_512);
			break;
		default:
		...
		switch(mode) {
			case MODE_224:
				for(j=0; j<7; j++) { printf("%08x", ((uint32_t*)h)[j]); }
				break;
			case MODE_256:
				for(j=0; j<8; j++) { printf("%08x", ((uint32_t*)h)[j]); }
				break;
			case MODE_384:
				//for(j=0; j<7; j++) { printf("%16x", ((uint64_t*)h)[j]); }
				break;
			case MODE_512:
				//for(j=0; j<8; j++) { printf("%16x", ((uint64_t*)h)[j]); }
				break;
			default:
				printf("How did you reach this code?\n");
				return 1;
